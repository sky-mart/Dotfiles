...
(defun mart/get-project-meta-dir ()
  (interactive)
...
(defun mart/gdb-current-tests ()
  (interactive)
  ;; find a directory with project.meta
  (setq moduledir (mart/get-project-meta-dir)
        modulename (file-name-nondirectory moduledir))
  (gdb (format "gdb -i=mi --args %s" (file-name-concat moduledir "build" "UnitTestBase" modulename)))
  )

(define-key c-mode-map (kbd "<f5>") 'mart/gdb-current-tests)
(define-key c++-mode-map (kbd "<f5>") 'mart/gdb-current-tests)
#+END_SRC
